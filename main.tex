  ...
  \begin{adjustbox}{minipage=19cm, center}
   \rowcolors{2}{gray!20}{white}
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}lXX}
      ...
      \texttt{storage} &
      The storage allocated for this job. &
      \texttt{\footnotesize "5GiB"} \newline \texttt{\footnotesize "10GiB"} \\

      \texttt{preemptible} &
      If \texttt{True}, this job is executed on ``preemptible'' or ``spot'' cores. &
      \texttt{\footnotesize True} \newline \texttt{\footnotesize False} \\

      \texttt{timeout} &
      The maximum time in seconds that a job may run. \texttt{None} indicates the job may run forever. &
      \texttt{\footnotesize None} \newline \texttt{\footnotesize 300} \\

      \texttt{cloudfuse} &
      A list of cloud storage (e.g. S3, GCS) buckets exposed in the filesystem of the job. The Boolean indicates if the mount is read-only. &
      \texttt{\footnotesize [("bucket1", "/bucket1", True)]} \\

      \texttt{always\_copy\_output} &
      Attempt to copy the output files into cloud storage even if the job fails. &
      \texttt{\footnotesize True} \newline \texttt{\footnotesize True} \\

      \texttt{env} &
      The environment variables and associated values for the job. &
      \texttt{\footnotesize \{"MKL\_NUM\_THREADS": "2"\}} \\

      \texttt{regions} &
      A set of permissible cloud regions in which to execute this job. \texttt{None} indicates all regions are permissible. &
      \texttt{\footnotesize None} \newline \texttt{\footnotesize \{"us-central", "us-east"\}} \\

      \texttt{inputs} &
      A list of symbolic files which are exposed in the filesystem of the job. &
      \\

      \texttt{ouputs} &
      A list of symbolic files which may appear in the inputs of jobs that depend on this job. &
      \texttt{\footnotesize None} \newline \texttt{\footnotesize \{"us-central", "us-east"\}} \\

    \end{tabularx}
  \end{adjustbox}
  \caption{The properties of a job.}
  \label{fig:job-properties}
\end{figure}
